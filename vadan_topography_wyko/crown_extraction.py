...
def extract_crown_profiles(data_processed_laser, Resolution):
    ...
    # THESE WILL HAVE X (1st column) and Y COORDINATES FOR A SET OF POINTS
    crown_profile = []
    xcrown_profile = []
    ...
    for i in range(len(data_processed_laser)):
        if -0.5 * Resolution <= data_processed_laser[i, 0] <= 0.5 * Resolution:
            if not np.isnan(data_processed_laser[i, 1]) and not np.isnan(
                data_processed_laser[i, 2]
            ):
    ...
    for i in range(len(data_processed_laser)):
        if -1 * Resolution <= data_processed_laser[i, 1] <= Resolution:
            if not np.isnan(data_processed_laser[i, 0]) and not np.isnan(
                data_processed_laser[i, 2]
            ):
